...
pairwiseCC <- function(d,i,j){
  ...
  ndCC <- length(dCC[[i]])
  gScat <- ggplot(d,aes_string(x=i,y=j)) + geom_point(size=.3,alpha=.3) + 
...
fCol <- which(names(fDF) %in% c('T1_HSstrength_FPM_SSDS','T2_HSstrength_FPM_SSDS','Spo11Oligo_strength','zcwpw1','H3K4me3_b6_kp1_peakStr','h3k4m3Lep','h3k4m3Zyg'))
fCC <- fDF[,fCol]
names(fCC) <- c('SSDS(1)','SSDS(2)','k4m3 12dpp','spo11','k4m3 Le','k4me Zy', 'Zcwpw1')

fCCok <- fCC[apply(fCC,1,function(x){if(sum(is.na(x))>0 | sum(x==0) > 0){FALSE}else{TRUE}}),]

ord <- c('SSDS(1)','SSDS(2)','spo11','k4m3 12dpp','k4m3 Le','k4me Zy', 'Zcwpw1')
df4CC <- fCCok[,ord]
...
for (nI in 1:length(ord)){
  for (nJ in 1:length(ord)){
    ...
    ccMat2[nI,nJ] <- lCC$cc
    ggsave(getIMGname(paste0(ord[nI],'_v_',ord[nJ],'_strength_Scatter','PNG',saveLocation = './'),gCC,width = 3,height = 3)
    ggsave(getIMGname(paste0(ord[nI],'_v_',ord[nJ],'_strength_Scatter','PNG',saveLocation = './'),gCC,width = 3,height = 3)
  }
}
